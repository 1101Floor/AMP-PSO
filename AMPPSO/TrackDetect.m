...
% 威胁检测
dim = USV.PointDim;                                         % 仿真环境维度
M = [USV.Menace.radar; USV.Menace.other]; % 威胁区

Threat = cell(USV.num, 1);                                 % 威胁结果树
Angle = cell(USV.num, 1);                                  % 转角检测结果树
MiniTraj = cell(USV.num, 1);                              % 最小航迹片段检测结果树
ProbPoint = cell(USV.num, 1);                           % 问题点  

L = cell(USV.num, 1);                                          % 航迹片段树（累加结构）
Time = cell(USV.num, 1);                                    % 到达各个点时间树                                          
      
L_mt = 0;                                                               % 所有无人船航迹之和
totalTime = zeros(USV.num, 1);                         % 所用时间
totalL = zeros(USV.num, 1);                                % 每个无人船航行距离
...
for i = 2 : USV.num
    ...
    for k = 1 : PointNum_i
        P1 = Track.P{i}(:, k)';  % K时刻 i 无人船位置
        t_i = Time{i, 1}(k);                  % K时刻 i 无人船时间
        for j = 1 : i-1
            ...
            for kj = 1 : PointNum_j
                ...
                if t_i <= t_j_r  &&  t_i >= t_j_l
                    flag = true;
                    P2 =  P_l + (t_i - t_j_l) / (t_j_r - t_j_l) * (P_r - P_l);% K时刻 J 无人船位置
                    % K时刻 j 无人船位置
                end
            end
            % ———————
...
report.L = totalL;                            %航行距离
report.time = totalTime;                %航行时间
